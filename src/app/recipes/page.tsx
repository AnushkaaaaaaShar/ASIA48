...
export default function RecipeGenerator() {
  ...
  const getRecipe = async () => {
    ...
    try {
      ...
      let text = response.text(); // Get the response as a string

       // Robustly extract JSON from the text, in case the model wraps it in markdown code blocks.
      // This regex looks for a block starting with ```json and ending with ```
      const jsonMatch = text.match(/```json\n([\s\S]*?)\n```/);
      if (jsonMatch && jsonMatch[1]) {
        text = jsonMatch[1]; // Use the extracted JSON string
      } else {
        // If no markdown block is found, assume the text is directly JSON.
        // This is important because the model might sometimes return pure JSON without fences.
        console.warn("JSON not wrapped in ```json``` block. Attempting direct parse.");
      }
